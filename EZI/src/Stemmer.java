...
class Stemmer {
	...
	public static ArrayList<String> read(String data) {
		data = data.toLowerCase();
		Stemmer s = new Stemmer();
		ArrayList<String> result = new ArrayList<String>();
		int di = 0;
		while (di < data.length()) {
			int ch = data.charAt(di);
			if (isLower(ch)) {
				int j = 0;
				while (true) {
					j++;
					if (di + j >= data.length() || !isLower(ch = data.charAt(di + j))) {
						for (int c = 0; c < j; c++)
							s.add(data.charAt(di + c));
						s.stem();
	...
	public static ArrayList<ArrayList<String>> readFile(String filename, boolean single) throws IOException {
